...
typedef struct {
    Obj obj;
    // The receiver’s type is Value even though methods can be called only on ObjInstances
    // Since the VM doesn't care what kind of receiver it has anyway, using Value means we don't
    // have to keep converting the pointer back to a Value when it gets passed to more general functions.
    Value receiver;
    ObjClosure* method;
} ObjBoundMethod;
